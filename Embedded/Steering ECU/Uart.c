 ...
 *********************************************************************************************************************/
#define TIMEOUT_VALUE 10000   // Timeout value in microseconds
/**********************************************************************************************************************
 *  LOCAL FUNCTIONS
 *********************************************************************************************************************/
 ...
 *                                    E_NOT_OK
 *******************************************************************************/
void uart_receive_string(char *receive_buffer, char* value_string)
{
    incoming = Uart_RX (UART_1);  // Receive data into buffer

    if (incoming == '-' || incoming == '+')
    {
        receive_buffer[receive_index] = incoming ;

        while(receive_index < 5)
        {
            incoming = Uart_RX (UART_1);  // Receive data into buffer
            receive_buffer[receive_index+1] = incoming ;
            receive_index++;

        }
    }
    receive_index = 0;  // Reset receive buffer for next message
    parse_message(receive_buffer , value_string); // Parse received data

    whole_num_1 = combineCharsToInt(value_string[1], value_string[2]);
    fraction_num_1 = combineCharsToInt(value_string[3], value_string[4]);
    if (value_string != NULL_PTR  &&
         (value_string[0] == '+' || value_string[0] == '-') &&
         (whole_num_1 >= 0 && whole_num_1 <= 99) &&
         (fraction_num_1 >= 0 && fraction_num_1 <= 99))
     {
        Number_1 = intToFloat(value_string[0], whole_num_1, fraction_num_1);
    }
}
...
void Float_to_chars_PID(float num1 , char* str)
{
    if (num1 < 0 )
    {
        Integer_Part_1 = (-1 * ((int) num1));
        Fractional_part_1 = (-1 * num1) - (float) Integer_Part_1;
        Integer_Fractional_part_1 = Fractional_part_1 * 100;
    }
    else if (num1 > 0 )
    {
        Integer_Part_1 = (int) num1;
        Fractional_part_1 = num1 - (float) Integer_Part_1;
        Integer_Fractional_part_1 = Fractional_part_1 * 100;
    }
    ...
    if (Integer_Part_1 < 10 )
    {
        str[1] = 0 + '0';
        str[2] = Integer_Part_1 + '0';
    }
    else if (Integer_Part_1 >= 10 )
    {
        // Extract the first digit
        str[1] = Integer_Part_1 / 10 + '0';

        // Extract the second digit
        str[2] = Integer_Part_1 % 10 + '0';
    }
    if (Integer_Fractional_part_1 < 10 )
    {
        str[3] = 0 + '0';
        str[4] = Integer_Fractional_part_1 + '0';
    }
    else if (Integer_Fractional_part_1 >= 10 )
    {
        // Extract the first digit
        str[3] = Integer_Fractional_part_1 / 10 + '0';

        // Extract the second digit
        str[4] = Integer_Fractional_part_1 % 10 + '0';
    }
